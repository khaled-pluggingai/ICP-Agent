...
export function HudButton({ 
  ...
  const getColors = () => {
    if (variant === "primary") {
      return {
        main: isDark ? "hsl(142, 84%, 60%)" : "hsl(142, 76%, 36%)", // Brand green
        gradient: isDark ? "hsl(142, 84%, 60%)" : "hsl(142, 76%, 36%)",
        text: isDark ? "text-emerald-300" : "text-emerald-700",
        glow: isDark ? "rgba(58, 245, 133, 0.3)" : "rgba(34, 197, 94, 0.2)",
        border: isDark ? "hsl(142, 84%, 60%)" : "hsl(142, 76%, 36%)"
      }
    } else {
      return {
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          ...
          >
            <HyperText
              ...
              className={`font-semibold select-none ${sizeStyles.textClass} ${colors.text}`}
